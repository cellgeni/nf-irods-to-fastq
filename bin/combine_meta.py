...
def make_unique(sample_fastq_name: str, unique_tags: set[int], fastq_names: list[str]) -> str:
    """
    ...
    if sample_fastq_name in fastq_names:
        ...
        sample_fastq_name = sample_fastq_name.replace(f'_S{sample_tag}_', f'_S{unique_tag}_')
        print(f"Filename changed from {sample_fastq_name.replace(f'_S{unique_tag}_', f'_S{sample_tag}_')} to {sample_fastq_name}")
...
def main():
    ...
    for filename in os.listdir(dirpath):
        with open(f'{dirpath}/{filename}', 'r') as file:
            ...
            sample_meta['fastq_name'] = make_unique(sample_meta['fastq_name'], unique_tags, fastq_names)
